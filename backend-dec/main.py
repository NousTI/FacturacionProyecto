...
from api.routes.configuracion_routes import router as configuracion_router
...
async def error_handling_middleware(request: Request, call_next):
    """
    Captura errores y devuelve respuestas consistentes {status, code, message, details, timestamp},
    ...
    try:
        ...
        logger.exception(
            "[INTERNAL_ERROR] Unhandled error %s %s user=%s rid=%s: %s",
            ctx["method"],
            ctx["path"],
            ctx["user_id"],
            ctx["request_id"],
        ...
        return JSONResponse(
            status_code=500,
            content=error_response("INTERNAL_ERROR", "Error interno del servidor", details=str(e) if settings.debug else None),
            headers={"X-Request-ID": request_id},
        )
