...
class RepositorioNotificaciones:
    ...
    def marcar_como_leida(self, id: UUID, user_id: UUID) -> Optional[dict]:
        ...
        query = "UPDATE sistema_facturacion.notificaciones SET leida = true WHERE id = %s AND user_id = %s RETURNING *"
        with self.db.cursor() as cur:
    ...
    def marcar_todas_leidas(self, user_id: UUID) -> bool:
        ...
        query = "UPDATE sistema_facturacion.notificaciones SET leida = true WHERE user_id = %s"
        with self.db.cursor() as cur:
            cur.execute(query, (str(user_id),))
            return True
    ...
    def contar_no_leidas(self, user_id: UUID) -> int:
        ...
        query = "SELECT COUNT(*) as count FROM sistema_facturacion.notificaciones WHERE user_id = %s AND leida = false"
        with self.db.cursor() as cur:
            cur.execute(query, (str(user_id),))
            row = cur.fetchone()
