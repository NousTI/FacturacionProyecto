...
class RepositorioUsuarios:
    ...
    def crear_usuario(self, data: dict) -> Optional[dict]:
        ...
        # Adaptar campos al nuevo esquema
        if 'password' in data:
        ...
        query = f"""
            INSERT INTO sistema_facturacion.users ({', '.join(fields)})
    ...
    def obtener_por_correo(self, correo: str) -> Optional[dict]:
        with self.db.cursor() as cur:
            cur.execute("SELECT * FROM sistema_facturacion.users WHERE email=%s", (correo,))
            row = cur.fetchone()
            return dict(row) if row else None

    def obtener_por_id(self, usuario_id: UUID) -> Optional[dict]:
        with self.db.cursor() as cur:
            cur.execute("SELECT * FROM sistema_facturacion.users WHERE id=%s", (str(usuario_id),))
            row = cur.fetchone()
            return dict(row) if row else None

    def listar_usuarios(self, empresa_id: Optional[UUID] = None, rol_id: Optional[UUID] = None) -> List[dict]:
        # Nota: La lógica de filtrado por empresa_id o rol_id podría requerir JOINs
        # con las nuevas tablas de perfiles y roles. Actualizamos solo el nombre de la tabla por ahora.
        query = "SELECT * FROM sistema_facturacion.users WHERE 1=1"
        params = []
        
        # if empresa_id: ... handled by profiles/roles now
    ...
    def eliminar_usuario(self, usuario_id: UUID) -> bool:
        query = "DELETE FROM sistema_facturacion.users WHERE id = %s RETURNING id"
        with db_transaction(self.db) as cur:
