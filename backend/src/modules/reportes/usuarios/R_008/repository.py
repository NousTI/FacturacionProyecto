...
class RepositorioR008:
    ...
    def obtener_kpis_cartera(self, empresa_id: UUID, fecha_inicio: Optional[str] = None, fecha_fin: Optional[str] = None) -> Dict[str, Any]:
        ...
        query = """
            SELECT
                ...
                COALESCE(SUM(cc.saldo_pendiente) FILTER (WHERE CURRENT_DATE - cc.fecha_vencimiento < 30), 0) as vencido_menor_30,
                COALESCE(SUM(cc.saldo_pendiente) FILTER (WHERE CURRENT_DATE - cc.fecha_vencimiento >= 30), 0) as cartera_critica
    ...
    def obtener_top_clientes_pendientes(self, empresa_id: UUID, fecha_inicio: Optional[str] = None, fecha_fin: Optional[str] = None, limit: int = 10) -> List[Dict[str, Any]]:
        """Top clientes con saldo pendiente y antigüedad, filtrados por fechas."""
        query = """
            SELECT
        ...
        query += " GROUP BY c.id, c.razon_social, u.nombres, u.apellidos ORDER BY saldo_total DESC LIMIT %s"
        params.append(limit)
