...
class RolRepository:
    ...
    def list_roles(self, empresa_id: UUID) -> List[dict]:
        """List roles for a specific empresa ID."""
    ...
    def update_rol(self, rol_id: UUID, data: dict) -> Optional[dict]:
        ...
        try:
            with db_transaction(self.db) as cur:
                ...
                query = f"UPDATE rol SET {', '.join(fields)} WHERE id = %s RETURNING *"
    ...
    def update_permission(self, rol_id: UUID, permission_id: UUID, data: dict) -> Optional[dict]:
        ...
        try:
            with db_transaction(self.db) as cur:
                ...
                query = f"UPDATE rol_permiso SET {', '.join(fields)} WHERE rol_id = %s AND permiso_id = %s RETURNING *"
                params.append(str(rol_id))
            ...
            print(f"Error updating permission assignment: {e}")
            return {"error": str(e)}
