...
async def test_creation():
    ...
    try:
        ...
        repo_cli = RepositorioClientes(db)
        repo_est = RepositorioEstablecimientos(db)
        repo_pun = RepositorioPuntosEmision(db)
        repo_emp = RepositorioEmpresas(db)
        repo_fac = RepositorioFacturas(db)
        ...
        serv_cli = ServicioClientes(repo_cli)
        serv_est = ServicioEstablecimientos(repo_est)
        serv_pun = ServicioPuntosEmision(repo_pun, serv_est) # FIX: pass serv_est
        serv_emp = ServicioEmpresas(repo_emp)
        ...
        core = ServicioFacturaCore(
            repo=repo_fac,
            cliente_service=serv_cli,
            establecimiento_service=serv_est,
            punto_emision_service=serv_pun,
            punto_emision_repo=repo_pun,
            empresa_service=serv_emp
        )
