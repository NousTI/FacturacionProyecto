...
import { EmpresaInfoCardComponent } from '../../../empresa/components/empresa-info-card/empresa-info-card.component';
import { EditEmpresaModalComponent } from '../../../empresa/components/edit-empresa-modal/edit-empresa-modal.component';
...
@Component({
  ...
  template: `
    <div class="config-empresa-wrapper">
      <!-- Loading State -->
      ...
      <!-- Content -->
      <div *ngIf="empresa" class="animate__animated animate__fadeIn">
        
        <!-- Banner de Inactividad -->
        <div *ngIf="!empresa.activo" class="alert alert-danger shadow-sm rounded-4 mb-4 p-4 d-flex align-items-center justify-content-between">
          <div class="d-flex align-items-center gap-3">
            <i class="bi bi-exclamation-octagon-fill fs-1"></i>
            <div>
              <h4 class="alert-heading fw-bold mb-1">Empresa Inhabilitada</h4>
              <p class="mb-0 fw-medium">Esta empresa ha sido desactivada por el administrador del sistema.</p>
            </div>
          </div>
      ...
      <div *ngIf="!loading && !empresa" class="empty-state shadow-sm rounded-4 p-5 text-center bg-white">
        ...
        <p class="text-muted">No pudimos recuperar los datos de la empresa.</p>
  ...
  styles: [`
    ...
    /* Copied from original empresa page styles for safety */
    .stats-compact-row {
      background: white;
      border-radius: 20px;
  ...
  handleUpdate(formData: any): void {
    ...
    updatableFields.forEach(field => {
      if (formData[field] !== undefined) {
        payload[field] = typeof formData[field] === 'string' ? formData[field].trim() : formData[field];
      }
    });
