...
@Component({
      ...
      <div class="modal-content-container shadow-lg" (click)="$event.stopPropagation()">
        ...
        <div class="detail-header">
          <div class="client-brand">
            <div class="avatar-large" [style.background]="getAvatarColor(proveedor.razon_social, 0.1)" [style.color]="getAvatarColor(proveedor.razon_social, 1)">
              {{ getInitials(proveedor) }}
            </div>
            <div class="brand-text">
              <h4>{{ proveedor.razon_social }}</h4>
              <div class="badges">
                <span class="status-badge" [ngClass]="proveedor.activo ? 'active' : 'inactive'">
                  {{ proveedor.activo ? 'Proveedor Activo' : 'Proveedor Inactivo' }}
                </span>
                <span class="type-badge" *ngIf="proveedor.dias_credito > 0">Con Crédito</span>
              </div>
            </div>
          </div>
              ...
              <div class="info-group">
                ...
                <div class="info-grid">
                  <div class="info-item">
                    ...
                    <span class="text-uppercase">{{ proveedor.tipo_identificacion }}</span>
                  </div>
                  <div class="info-item">
                    <label>Número</label>
                    <span class="fw-bold">{{ proveedor.identificacion }}</span>
                  </div>
                  <div class="info-item full">
                    <label>Nombre Comercial</label>
                    <span>{{ proveedor.nombre_comercial || 'N/A' }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="info-group">
                ...
                <div class="info-grid">
                  <div class="info-item full">
                    ...
                    <span class="email-text">{{ proveedor.email }}</span>
                  </div>
                  <div class="info-item">
                  ...
                  <div class="info-item">
                    ...
                    <span>{{ proveedor.ciudad || '—' }}, {{ proveedor.provincia || '—' }}</span>
                  </div>
                  <div class="info-item full">
                    <label>Dirección Fiscal</label>
                    <span class="address-text">{{ proveedor.direccion || 'Sin dirección registrada' }}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="col-md-5">
              <div class="metric-card">
                ...
                <div class="metric-item">
                  ...
                  <span class="m-value">{{ proveedor.dias_credito || 0 }} Días</span>
                </div>
              </div>
              ...
              <div class="audit-info">
                ...
                <div class="audit-row">
                  ...
                  <span class="a-value">{{ proveedor.created_at | date:'dd/MM/yyyy' }}</span>
                </div>
                <div class="audit-row" *ngIf="proveedor.updated_at">
                  <span class="a-label">Última Actualización</span>
                  <span class="a-value">{{ proveedor.updated_at | date:'dd/MM/yyyy' }}</span>
                </div>
              </div>
            </div>
          </div>
  ...
  styles: [`
    ...
    .modal-content-container { background: white; border-radius: 28px; width: 100%; max-width: 850px; max-height: 90vh; display: flex; flex-direction: column; overflow: hidden; border: 1px solid #f1f5f9; }
    ...
    .type-badge { background: #eff6ff; color: #1e40af; padding: 0.4rem 0.8rem; border-radius: 8px; font-size: 0.75rem; font-weight: 700; }
    ...
    .metric-card { background: #1e293b; border-radius: 24px; padding: 2rem; color: white; margin-bottom: 2rem; }
...
export class ProveedorDetailModalComponent implements OnInit, OnDestroy {
  ...
  getInitials(proveedor: Proveedor): string {
    if (!proveedor.razon_social) return '??';
    return proveedor.razon_social.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase();
  }
