          ...
          <table class="table mb-0 align-middle">
            <thead>
              <tr>
                ...
                <th class="text-end" style="width: 80px" *ngIf="canAccess">Acciones</th>
              </tr>
            </thead>
            <tbody>
              <tr *ngFor="let empresa of empresas">
                ...
                <td class="text-end" *ngIf="canAccess">
                  <div class="dropdown">
                    <button 
                    ...
                    <ul class="dropdown-menu dropdown-menu-end shadow-premium-lg border-0 p-2 rounded-4" [attr.aria-labelledby]="'actions-' + empresa.id">
                      <li>
                        <a class="dropdown-item rounded-3 py-2 text-corporate fw-bold" href="javascript:void(0)" (click)="onAction.emit({type: 'access', empresa})">
...
export class VendedorEmpresaTableComponent {
    ...
    getAvatarColor(name: string, opacity: number): string {
        ...
        for (let i = 0; i < name.length; i++) {
            if (name.charCodeAt(i)) hash = name.charCodeAt(i) + ((hash << 5) - hash);
        }
