                ...
                <!-- Bars Container -->
                 <div class="bars-container d-flex align-items-end justify-content-between h-100 w-100 px-2 position-relative" style="z-index: 1;">
                    <div *ngFor="let item of data" 
                      ...
                      <!-- Bar Background (Previous Period Comparison) -->
                      <div *ngIf="item.value_prev !== undefined" 
                           class="bar-prev position-absolute rounded-pill" 
                           [style.height.%]="getBarHeight(item.value_prev || 0)"
                           [style.width.%]="(100 / data.length) * 0.4"
                           style="background: #e2e8f0; margin-bottom: 0; opacity: 0.5; z-index: -1;">
                      </div>

                      <div class="bar rounded-pill w-50 position-relative group" 
                           [style.height.%]="getBarHeight(item.value)"
                           [style.background]="barColor"
                           [title]="item.label + ': ' + item.value + (item.value_prev ? ' (Anterior: ' + item.value_prev + ')' : '')">
                           
                           <!-- Tooltip -->
                           <div class="tooltip-val position-absolute start-50 translate-middle-x badge bg-dark text-white rounded-pill px-2 py-1"
                                style="bottom: 100%; margin-bottom: 5px; opacity: 0; transition: opacity 0.2s; white-space: nowrap; z-index: 10;">
                              {{ item.value | number }}
                              <span *ngIf="item.value_prev" style="font-size: 0.6rem; display: block; opacity: 0.8;">Prev: {{ item.value_prev | number }}</span>
                           </div>
                      </div>
                    </div>
                 </div>
